...
class CopernicusDEM:
    ...
    def _create_reading_grid(self):
        ...
        df = pd.DataFrame(raster_grid,
                          columns=[self.col_grid_xmin, self.col_grid_ymin, self.col_grid_xmax, self.col_grid_ymax])
    ...
    def _project_points(self, df, lat_col, lon_col, input_crs):
        ...
        df[[self.col_proj_x, self.col_proj_y]] = np.column_stack(
            projection.transform(df[lat_col].values, df[lon_col].values))
...
def add_elevation_to_gpx(gpx_filename, output_filename, copernicus):
    gdf = gpd.read_file(gpx_filename, layer='track_points')
    gdf.dropna(axis=1, inplace=True)

    coordinates = gdf.get_coordinates()

    points = copernicus.get_elevation(coordinates, lat_col='y', lon_col='x')
    gdf['ele'] = points['elevation']
